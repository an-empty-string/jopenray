...
public class SshAdapter implements InputListener, Term {
	...
	public void start(ThinClient thinClient, Session session) {
		...
		try {
			...
			String hadad = null;
			if(session.getServer().contains(":")) {
				hadad = session.getServer().split(":")[0];
				port = Integer.parseInt(session.getServer().split(":")[1]);
			}
			else {
				hadad = session.getServer();
			}
			jschsession = JSchSession.getSession(session.getLogin(), session
					.getPassword(), hadad, port, ui, proxy);
